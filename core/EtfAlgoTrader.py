# from : https://github.com/INVESTAR/StockAnalysisInPython/blob/master/08_Volatility_Breakout/ch08_03_EtfAlgoTrader.py
# video : https://youtu.be/5bTxyBeOVkA 
...
# module
sys.path.append("C:\\Users\\user\\OneDrive\\바탕 화면\\DEV\\stock-auto\\extentions")
# print(sys.path)
from slack_bot import post_message
from crawl import get_target_etf
...
def dbgout(message, is_block=False, block_data=None):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    post_message('#stock-auto-log', strbuf, is_block=is_block, block_data=block_data)
...
def get_target_price(code):
    ...
    try:
        ...
        # change_value가 변동성 돌파 전략에서 K 값임 
        # 낮을 수록 변동이 작아도 매수가 된다 -> 정말 낮으면 웬만한 가격 변동에서 매수!
        change_value = 0.5
        target_price = today_open + (lastday_high - lastday_low) * change_value
        return target_price
...
def buy_etf(code):
    """인자로 받은 종목을 최유리 지정가 FOK 조건으로 매수한다. FOK는 전량 체결되지 않으면 주문 자체를 취소"""
    try:
        # 매수 한 종목을 또 매수하면 안되기 때문에 매수 종목을 리스트에 담음
...
def sell_all():
    """보유한 모든 종목을 최유리 지정가 IOC 조건으로 매도한다. IOC는 체결 후 남은 수량 취소"""
    try:
        cpTradeUtil.TradeInit()
        acc = cpTradeUtil.AccountNumber[0]       # 계좌번호
        accFlag = cpTradeUtil.GoodsList(acc, 1)  # -1:전체, 1:주식, 2:선물/옵션   
...
def main():
    ...
    try:
        ...
        # 종목 대상으로 자동 매매 진행 - ETF 가 유리하다! 
        symbol_dict = get_target_etf(5) # 다음 주식에서 가져온 STOCK 정보들
        symbol_list = symbol_dict['symbol_list'] # 주식 코드 리스트!
        if len(symbol_list) < 1:
            printlog('종목 코드 가져오기 에러 재시도 :', datetime.now().strftime('%m/%d %H:%M:%S'))
            time.sleep(5)
            symbol_list = get_target_etf(5)
            if len(symbol_list) < 1:
                sys.exit(0)

        # 종목 대상 체크 
        print(symbol_dict)
        dbgout(f"", True, {"symbol_list": str(symbol_dict)}) 

        bought_list = []                # 코드 돌아가며 매수 완료된 종목 리스트
        target_buy_count = 5            # 매수할 종목 수 (대상 종목 여러갤 때 최대 몇개까지)
        buy_percent = 0.25              # 가용자금에서 몇 퍼센트를 살 것이냐 
        ...
        while True:
            # 자동 매매 time setting
            t_now = datetime.now()  # now 기준
            t_9 = t_now.replace(hour=9, minute=0, second=0, microsecond=0)
            t_start = t_now.replace(hour=9, minute=5, second=0, microsecond=0)  # LP(유동성 공급자) 활동시간, 9:5 ~ 15:20
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0) # 15:15 분으로 판매 시작 시간 
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0)  # 자동매매 종료 시간
            ...
            # 혹시나 남아있는 주식이 있으면 모두 팔아라! 
            if t_9 < t_now < t_start and soldout == False:
                soldout = True
                sell_all()

            # t_start(AM 09:05) ~ t_sell(PM 03:15) : 매수
            ...
            # 변동성 돌파 전략으로 - 아래 시간에는 일괄 매도
            # t_sell(PM 03:15) ~ t_exit(PM 03:20) : 일괄 매도
            if t_sell < t_now < t_exit:  
                if sell_all() == True:
                    dbgout('`sell_all() returned True -> self-destructed!`')
                    sys.exit(0)

            # t_exit(PM 03:20) : 프로그램 종료
            if t_exit < t_now: 
